...
1. **创建数据库**
   - 打开 MariaDB/MySQL 数据库管理工具（Navicat / SQLyog / MySQL Workbench）
...
1. **创建 Spring Boot 项目**
   - 参考完整手册 **步骤 2.1**，使用 VS Code（Spring Initializr 扩展或 start.spring.io）创建项目
...
2. **配置 Maven 依赖**
    ...
    - 关键依赖说明：
       ...
       - `mariadb-java-client`：MariaDB 数据库驱动
...
1. 在 VS Code 中打开项目，找到 `LibraryApplication.java`（主启动类）
2. 使用 Maven 任务（如 `mvn spring-boot:run`）或 VS Code 的 Spring Boot Dashboard 启动
3. 观察终端，确认启动成功（看到 "Started LibraryApplication in X seconds"）
...
**解决方案**：
1. 检查 MariaDB 服务是否启动（Arch Linux：`systemctl status mariadb.service`）
...
**现象**：VS Code Maven 插件提示 "Maven import failed"

**解决方案**：
1. 检查网络连接
2. 配置阿里云 Maven 镜像（在 `pom.xml` 同级目录创建 `settings.xml`）
3. 在 VS Code 命令面板执行 “Maven: Reload All Maven Projects” 或在终端运行 `mvn -U clean package`
